...
def build_devices_workbook(devices):
    ...
    headers = [
        ...
        'System Brand', 'System Model', 'CPU', 'CPU Serial', 'CPU Manufacturer',
        'Cores', 'Threads', 'Clock Speed', 'Architecture',
        'Motherboard', 'Motherboard Serial',
        'BIOS Serial', 'RAM Total', 'RAM Modules', 'Storage',
        'Monitor Brand', 'Monitor', 'Monitor Serial', 'Screen Size', 'Resolution', 'Monitor Mfg Date',
        'Mouse Brand', 'Mouse', 'Keyboard Brand', 'Keyboard', 'UPS', 'Asset Tag', 'Vendor', 'Purchase Date',
        'Registered', 'Active',
    ]
    ...
    for device in devices:
        ...
        values = [
            ...
            asset.system_brand if asset else '',
            asset.cpu_name if asset else '', asset.processor if asset else '',
            asset.cpu_serial if asset else '',
            asset.cpu_manufacturer if asset else '',
            asset.cpu_cores if asset else '', asset.cpu_threads if asset else '',
            asset.cpu_clock_speed if asset else '', asset.cpu_architecture if asset else '',
            ...
            asset.monitor_manufacturer if asset else '',
            asset.monitor_name if asset else '', asset.monitor_serial if asset else '',
            asset.monitor_screen_size if asset else '', asset.monitor_resolution if asset else '',
            asset.monitor_manufacture_date if asset else '',
            asset.mouse_brand if asset else '', asset.mouse_name if asset else '',
            asset.keyboard_brand if asset else '', asset.keyboard_name if asset else '',
