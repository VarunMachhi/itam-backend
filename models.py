...
class Device(models.Model):
    ...
    status = models.CharField(max_length=20, choices=ASSET_STATUS_CHOICES, default='active',
                               help_text="Lifecycle status of this physical device, independent of "
...
class DeviceAssignment(models.Model):
    """One row per continuous period a specific Device was assigned to a
    specific Employee at a specific Branch. Device.employee/Device.branch
    (above) only ever hold the CURRENT assignment -- editing them directly
    has no memory of what came before. This model is where that history
    actually lives: 'Rahul was previously assigned to PC-001 at Branch A,
    then transferred to PC-025 at Branch B on 2026-08-12.'

    Records are created automatically whenever a sync or registration
    detects the employee/branch on a device has genuinely changed (see
    core.assignment_tracking.record_assignment_if_changed), AND can be
    created directly by an admin through /admin/core/deviceassignment/add/
    as the deliberate 'Transfer Employee' action -- saving a new row here
    is what performs a transfer; it automatically closes out whatever
    assignment was previously active for that device."""
    ...
    branch = models.ForeignKey(Branch, on_delete=models.SET_NULL, null=True, blank=True,
                                related_name='device_assignments')
    assignment_type = models.CharField(max_length=10, choices=ASSIGNMENT_TYPE_CHOICES, default='primary')
    ...
    def __str__(self):
        ...
        return f"{self.device.hostname} \u2192 {self.employee or 'unassigned'} @ {self.branch or '?'} ({status})"
