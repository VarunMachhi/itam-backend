...
from core.models import AppRelease, Asset, AssetChangeLog, Branch, Command, Device, DeviceAssignment, Employee, Notification, Printer, SyncLog
...
class AssignmentHistoryInline(admin.TabularInline):
    """Read-only history of every employee/branch this device has ever
    been assigned to (\u00a713/\u00a714) -- shown on the Device page so 'who had
    this PC before, and when' is visible without leaving that screen."""
    model = DeviceAssignment
    extra = 0
    can_delete = False
    fields = ['employee', 'branch', 'assignment_type', 'is_active', 'assigned_at', 'unassigned_at', 'notes']
    readonly_fields = fields
    ordering = ['-assigned_at']

    def has_add_permission(self, request, obj=None):
        return False  # transfers happen via /admin/core/deviceassignment/add/, not inline here


@admin.register(Device)
class DeviceAdmin(admin.ModelAdmin):
    list_display = ['hostname', 'employee', 'branch', 'status', 'device_type', 'status_badge',
                     'app_version', 'version_status', 'last_seen', 'last_sync', 'is_active']
    list_filter = ['branch', 'device_type', 'status', 'is_active']
    search_fields = ['hostname', 'mac_address', 'employee__name', 'id']
    readonly_fields = ['id', 'api_key', 'registered_at', 'last_seen', 'last_sync']
    inlines = [AssetInline, AssignmentHistoryInline]
...
class DeviceAssignmentAdmin(admin.ModelAdmin):
    """This IS the 'Transfer Employee' function from \u00a714: pick a device,
    an employee, a branch, save -- and this closes out whatever assignment
    was previously active for that device, keeps Device.employee/branch in
    sync with the new assignment, and preserves the old one as history
    (never deleted, just marked inactive with an end date)."""
    list_display = ['device', 'employee', 'branch', 'assignment_type', 'is_active', 'assigned_at', 'unassigned_at']
    list_filter = ['assignment_type', 'is_active', 'branch']
    search_fields = ['device__hostname', 'employee__name']
    ...
    def save_model(self, request, obj, form, change):
        if not change:
            ...
            obj.is_active = True
            # Close out whatever was previously active for this same device --
            # never more than one active assignment per device at a time.
            DeviceAssignment.objects.filter(device=obj.device, is_active=True).update(
                is_active=False, unassigned_at=timezone.now())
        super().save_model(request, obj, form, change)

        if not change:
            # Keep the denormalized "current assignment" fields on Device
            # in sync with what admin just recorded here.
            device = obj.device
            device.employee = obj.employee
            device.branch = obj.branch
            device.save(update_fields=['employee', 'branch'])
